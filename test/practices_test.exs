defmodule DeepSeekHarness.PracticesTest do
  ...
  test "saves and loads practices in .lmml format", %{tmp_dir: tmp_dir, global_dir: global_dir} do
    ...
    {:ok, _content} = Practices.save_practices("testelixir", items, [target: :local, global_dir: global_dir], tmp_dir)
  ...
  test "merges global and local practices", %{tmp_dir: tmp_dir, global_dir: global_dir} do
    ...
    Practices.save_practices("testlang", global_items, [target: :global, global_dir: global_dir], tmp_dir)
    Practices.save_practices("testlang", local_items, [target: :local, global_dir: global_dir], tmp_dir)

    merged = Practices.load_practices("testlang", tmp_dir, global_dir: global_dir)
    assert merged.items == ["Global practice item 1", "Shared practice item", "Local practice item 2"]
  end

  test "adds and deletes practice items", %{tmp_dir: tmp_dir, global_dir: global_dir} do
    Practices.save_practices("testelixir2", ["Initial practice"], [target: :local, global_dir: global_dir], tmp_dir)

    {:ok, _} = Practices.add_practice("testelixir2", "New added practice", tmp_dir)
    loaded = Practices.load_practices("testelixir2", tmp_dir, global_dir: global_dir)
    assert "New added practice" in loaded.items

    {:ok, _} = Practices.delete_practices("testelixir2", [1], tmp_dir)
  ...
  test "squeezes practices from exemplary project directory", %{tmp_dir: tmp_dir, global_dir: global_dir} do
    ...
    {:ok, _} = Practices.squeeze_practices("testelixir3", [proj_dir], cwd: tmp_dir, global_dir: global_dir)

    loaded = Practices.load_practices("testelixir3", tmp_dir, global_dir: global_dir)
    assert loaded.language == "testelixir3"
    assert length(loaded.items) > 0
  end

  test "builds prompt preamble with language practices", %{tmp_dir: tmp_dir, global_dir: global_dir} do
    File.write!(Path.join(tmp_dir, "mix.exs"), "# mix")
    Practices.save_practices("elixir", ["Practice line for test"], [target: :local, global_dir: global_dir], tmp_dir)
