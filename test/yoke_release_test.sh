#!/usr/bin/env bash
#
# test/yoke_release_test.sh
#
# Tests for the `yoke` launcher's release freshness detection and rebuild
...
# Extract the release_needs_build + build_release functions from the yoke
...
extract_helpers() {
  ...
  local out="$2"
  {
    ...
    echo 'set -u'
    awk '/^release_needs_build\(\) \{/,/^}/' "$yoke"
    awk '/^build_release\(\) \{/,/^}/' "$yoke"
  } > "$out"
  chmod +x "$out"
}
...
fixture() {
  local base="$1"
  mkdir -p "${base}/lib/yoke" "${base}/config" "${base}/priv"
  mkdir -p "${base}/_build/prod/rel/yoke/bin"
  echo "mix.exs" > "${base}/mix.exs"
  echo "mix.lock" > "${base}/mix.lock"
  echo "config" > "${base}/config/config.exs"
  echo "code" > "${base}/lib/yoke/sample.ex"
...
echo "== yoke release freshness tests =="
...
touch -d "2024-06-01 00:00:00" "${FIXTURE}/lib/yoke/sample.ex"
extract_helpers "${PROJECT_ROOT}/yoke" "${WORK_DIR}/helpers.sh"
if SCRIPT_DIR="$FIXTURE" bash -c 'source "$0"; release_needs_build "$1"' "${WORK_DIR}/helpers.sh" "$REL"; then
...
# ---------------------------------------------------------------
echo "Test 4: YOKE_REBUILD=1 forces rebuild regardless of timestamps"
# ---------------------------------------------------------------
fixture "${WORK_DIR}/t4"
FIXTURE="${WORK_DIR}/t4"
REL="${FIXTURE}/_build/prod/rel/yoke/bin/yoke"
...
touch -d "2023-12-01 00:00:00" \
  ...
  "${FIXTURE}/priv/asset.txt"
extract_helpers "${PROJECT_ROOT}/yoke" "${WORK_DIR}/helpers.sh"
if SCRIPT_DIR="$FIXTURE" YOKE_REBUILD=1 bash -c 'source "$0"; release_needs_build "$1"' "${WORK_DIR}/helpers.sh" "$REL"; then
  pass "YOKE_REBUILD=1 forces rebuild"
else
  fail "expected YOKE_REBUILD=1 to force rebuild"
fi
...
if echo "$output" | grep -q "MIX_ENV=prod CMD=release yoke --overwrite"; then
  pass "build_release invokes mix release yoke --overwrite"
else
...
if [[ "$failures" -eq 0 ]]; then
  echo "✅ All yoke release tests passed."
  exit 0
else
  echo "❌ $failures yoke release test(s) failed."
  exit 1
fi
