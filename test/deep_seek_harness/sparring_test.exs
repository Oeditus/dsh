defmodule DeepSeekHarness.SparringTest do
  ...
  setup do
    ...
    File.write!(Path.join(tmp_dir, "project/lessons.md"), "Cache invalidation causes stale state.")
  ...
  test "prepares socratic and adversarial sparring with prior-art sweep", %{tmp_dir: tmp_dir} do
    {:ok, sweep_fmt, prompt} = Sparring.prepare_sparring("Cache state", cwd: tmp_dir, mode: :socratic)
    ...
    {:ok, _sweep_fmt2, adv_prompt} = Sparring.prepare_sparring("Cache state", cwd: tmp_dir, mode: :adversarial)
