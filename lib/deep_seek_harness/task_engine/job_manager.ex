defmodule DeepSeekHarness.TaskEngine.JobManager do
  @moduledoc """
  ...
  captures stdout/stderr to disk (`.dsh/jobs/<id>.log`), surfaces active jobs in
  ...
  @doc """
  Starts a bash command in the background, logging stdout/stderr to `.dsh/jobs/<id>.log`.
  """
  def start_job(command, opts \\ []) when is_binary(command) do
    ensure_started()
    cwd = Keyword.get(opts, :cwd, File.cwd!())
    id = "job_#{System.unique_integer([:positive])}"
    jobs_dir = Path.join(cwd, ".dsh/jobs")
    ...
    task =
      Task.Supervisor.async_nolink(
        DeepSeekHarness.TaskEngine.TaskSupervisor,
        fn ->
