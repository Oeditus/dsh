defmodule DeepSeekHarness.Brain.Session do
  ...
  - STRICT TOOL PREFERENCE (RAGEX CODE INTELLIGENCE FIRST):
    ...
    - NEVER use raw `bash` commands (such as `grep`, `find`, `sed`, `cat`, `head`, `tail`, `xargs`) or generic text tools for code searching, symbol finding, or file viewing when Ragex tools are available.
    - Use `bash` ONLY for executing build/test commands, running local binaries/scripts, git operations, or system commands where no suitable dedicated or Ragex MCP tool is available.
