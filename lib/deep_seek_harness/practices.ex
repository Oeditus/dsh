defmodule DeepSeekHarness.Practices do
  @moduledoc """
  Manages system-global (~/.dsh/practices/<language>.lmml) and project-local
  (.dsh/practices/<language>.lmml) "Good Practices" for programming languages in .lmml format.

  When a project in a new language is first seen by dsh, it prompts the user to
  ...
  and local sources and injected into prompt context whenever dsh works with code
  in that language.
  """
  require Logger
  ...
  @doc "Returns the directory path for system-global practices (~/.dsh/practices)."
  def global_practices_dir(opts \\ []) do
    Keyword.get(opts, :global_dir) || Path.expand("~/.dsh/practices")
  end

  @doc "Returns the directory path for project-local practices (.dsh/practices)."
  def local_practices_dir(cwd \\ ".") do
    Path.join([cwd, ".dsh", "practices"])
  end
  ...
  Saves to both global (~/.dsh/practices/<lang>.lmml) and project-local (.dsh/practices/<lang>.lmml)
  ...
  def render_narrative(language, items, manifest) do
    ...
    The following good practices are maintained for projects using #{lang_title}. They are automatically injected into DSH prompt context. You can edit this list directly or use `/practices` in DSH REPL.
  ...
  def squeeze_practices(language, project_paths, opts \\ []) do
    ...
    items =
      if code_samples != "" and api_key_available?() do
        ...
        case DeepSeekHarness.Client.DeepSeekAPI.chat_completion(
  ...
  If a project language is first seen (no practices exist yet in ~/.dsh or .dsh):
    - Asks the user to point to exemplary project paths.
    - Squeezes practices from those projects (or uses defaults if skipped).
    - Saves practices to global and project-local .dsh directories.
  """
  def ensure_practices_for_workspace(cwd \\ ".", opts \\ []) do
    ...
    Enum.each(languages, fn lang ->
      unless has_practices?(lang, cwd) do
        paths =
          if interactive? and not DeepSeekHarness.Config.god_mode?() do
            question =
              "Language '#{lang}' is new to dsh. Please enter path(s) to exemplary #{lang} project(s) to learn good practices from (or hit Enter for standard defaults):"
