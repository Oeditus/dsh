defmodule DeepSeekHarness.Sweep do
  @moduledoc """
  Multi-corpus prior-art search engine for DeepSeek Harness.
  ...
  3. `project/lessons.md` & `.dsh/lessons.md` - "has this bitten us before?"
  ...
  def run(tokens, opts) when is_list(tokens) do
    ...
    corpora_specs = [
      %{
        ...
        paths: [Path.join(cwd, "project/workflow"), Path.join(cwd, ".dsh/workflows")]
      },
      %{
      ...
      %{
        ...
        paths: [Path.join(cwd, "project/lessons.md"), Path.join(cwd, ".dsh/lessons.md")]
      },
      %{
  ...
  def resolve_vault_root(cwd \\ ".") do
    ...
    if env_vault && env_vault != "" && File.dir?(env_vault) do
      ...
      config_paths = [
        ...
        Path.join(cwd, ".dsh/config.json"),
        Path.expand("~/.dsh/config.json")
      ]
