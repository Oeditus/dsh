defmodule Yoke.Workflow.Steps.Commit do
  @moduledoc """
  Step ⑦: mirrors Yoke's own "please commit" convention -- format the
  ...
  Stages changes itself (excluding `.yoke/`) instead of going through
  `Git.commit/2`'s blanket `git add .`, so this workflow's own internal
  bookkeeping under `.yoke/workflows/runs/` -- state, transcripts, split
  plans -- can never end up committed into the user's history, even in a
  repository that doesn't already gitignore `.yoke/` (as this project's
  own `.gitignore` does).
  """
  @behaviour Yoke.Workflow.Step
  ...
  def run(context, _params) do
    ...
    stage_excluding_yoke(context.cwd)
        ...
        case commit(context.cwd, message) do
          {:ok, out} ->
            ...
            if item = Map.get(context, :pipeline_item) do
              Yoke.WorkflowIndex.close_out_active(item.slug, cwd: context.cwd)
            end
  ...
  defp stage_excluding_yoke(cwd) do
    System.cmd("git", ["add", "-A", "--", ".", ":!.yoke"], cd: cwd, stderr_to_stdout: true)
  end
