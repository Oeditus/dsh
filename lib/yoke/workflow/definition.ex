defmodule Yoke.Workflow.Definition do
  @moduledoc """
  Loads, validates, and discovers customizable Yoke workflow definitions.
  ...
  discovered the same way `Yoke.Skill.Manager` discovers
  skills -- a two-tier lookup (workspace `.yoke/workflows/definitions/`,
  then global `~/.yoke/workflows/definitions/`) -- plus a third,
  lowest-priority tier of definitions bundled with Yoke itself (currently
  ...
  `Yoke.Rules` seeds `@default_rules` into `.yoke/rules.json`.
  """
  require Logger

  alias Yoke.Rules
  alias Yoke.Workflow.Json
  ...
  @bundled %{
    "elixir" => %{
      ...
      "steps" => [
        %{"type" => "branch", "branch_prefix" => "yoke/elixir"},
  ...
  @doc "Returns the names of workflows bundled with Yoke itself."
  ...
  def definitions_dir(cwd \\ "."), do: Path.join(cwd, ".yoke/workflows/definitions")

  @doc "Global (cross-project) directory holding user/custom workflow definitions."
  def global_definitions_dir, do: Path.expand("~/.yoke/workflows/definitions")
  ...
  # duplicate rules in `.yoke/rules.json`.
