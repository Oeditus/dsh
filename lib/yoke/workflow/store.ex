defmodule Yoke.Workflow.Store do
  @moduledoc """
  Disk persistence for workflow runs, mirroring the conventions already
  established by `Yoke.Brain.SessionStore`: one directory per
  run under `.yoke/workflows/runs/<run_id>/`, a resumable `state.json`
  ...
  def runs_dir(cwd \\ "."), do: Path.join(cwd, ".yoke/workflows/runs")
